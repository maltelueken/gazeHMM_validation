...
author: "Malte Lüken, Simon Kucharský, & Ingmar Visser"
affiliation: "University of Amsterdam"
date: "05.05.2020"
output: pdf_document
---

In this document, we are preregistering a simulation study to estimate the parameter recovery of a hidden Markov model (HMM) which is part of our recently developed algorithm gazeHMM. The purpose of this algorithm is to classify eye movements into distinct events (e.g., fixations and saccades). The full algorithm will be available on Git hub upon publication.
...
The HMM uses an expectation-maximization algorithm to estimate parameters under the hidden states. The parameters of each response model (except for the uniform distribution) are estimated with a spectral projected gradient method and Barzilai-Borwein step lengths.
...
In the first part of the simulation study, we will estimate parameter recovery under ideal conditions. These imply an average sample size of $N = 2500$ samples per data set, no noise added to the data, and no missing samples. The parameters that we will use to generate the data are displayed in Table 1. We will vary one parameter at a time on the given interval and set the other parameters to their fixed value. For two states, 10 parameters will be varied one at a time, for three states 15, and for four states 20 parameters. This simulation will result in $D_{total} = 100 \times (10+15+20) = 4500$ recoveries.
