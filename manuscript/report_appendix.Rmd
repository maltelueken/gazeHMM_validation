...
tab.exp.tr <- cbind(1:4, matrix(round(backtrans(getpars(A2017.fit[[3]][[3]][[4]]$model))[5:20], 2), 4, 4, byrow = T))

apa_table(tab.exp.tr,
          col.names = c("From\nstate", as.character(1:4)),
          col_spanners = list("To state" = c(2,5)),
...
tab.exp.resp <- cbind(1:4, matrix(round(backtrans(getpars(A2017.fit[[3]][[3]][[4]]$model))[21:44], 2), 4, 6, byrow = T))
