...
In sum, HMMs seem to be a promising method for classifying eye movements. Nevertheless, the existing HMM algorithms each have their weaknesses that could be improved upon.  
...
The following section will describe gazeHMM and the underlying generative model in detail. Then, we will present the parameter recovery of the HMM and show how the algorithm performs compared to other eye movement event classification algorithms with regard to the agreement to human coding. Finally, we will discuss these results and propose directions in which gazeHMM and other HMM algorithms could be improved.
...
p1 <- ggplot(toPlot, aes(x = t, x.va)) + 
  ...
  geom_point(aes(y = x.va), color = "darkred", size = 1) + 
  geom_point(aes(y = y.va), color = "darkblue", size = 1) +
...
p3 <- ggplot(toPlot, aes(x = t, y = vel)) + 
  geom_line() + 
  geom_point(aes(color = event.pre), size = 1) +
  scale_y_continuous(name = "Velocity (deg/s)", limits = c(0, 250), breaks = pretty(c(0, 250)))

p4 <- ggplot(toPlot, aes(x = t, y = vel)) + 
  geom_line() + 
  geom_point(aes(color = event.post), size = 1) +
  scale_y_continuous(name = "Velocity (deg/s)", limits = c(0, 250), breaks = pretty(c(0, 250)))
