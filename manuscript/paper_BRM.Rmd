...
alg.names <- c(alg.names[1:2], "gazeHMM-3", "gazeHMM-4", alg.names[3:length(alg.names)])
...
A2017.events <- lapply(1:4, function(e) {
  lapply(1:length(A2017), function(stim) {
    lapply(alg.names, function(alg) {
      ...
      return(data.frame(mean(dur, na.rm = T), sd(dur, na.rm = T), length(dur)))
    }) %>% reduce(rbind)
  })
})
...
A2017.rmsd <- lapply(A2017.events, function(event) {
  ...
  out <- cbind(alg.names, rmsd, stringsAsFactors = FALSE)[-7,]
...
A2017.kappa <- cbind(alg.names, lapply(1:4, function(event) {
  lapply(1:length(A2017), function(stim) {
    ...
    df <- cbind(reduce(A2017[[stim]], rbind)[,c(6,7)], gazeHMM.3, gazeHMM.4, reduce(A2017[[stim]], rbind)[,c(8:18)])
...
# stimuliTypes <- c("dots", "img", "video")
# 
# kappaMat <- list()
# 
# for(i in 1:6) {
#   
#   kappaMat[[i]] = matrix(0, 2, 3)
#   
#   for(j in 1:length(stimuliTypes)) {
#     
#     events2 <- A2017[[stimuliTypes[j]]]
#     
#     numTot <- 0
#     
#     for (q in 1:length(events2)) {
#       
#       numTot <- numTot + nrow(events2[[q]])
#       
#     }
#    
#     event_vector <- matrix(0, numTot, 2)
#     
#     idx_start <- 1
#     idx_stop <- 0
#     
#     for (q in 1:length(events2)) {
#       
#       idx_stop <- idx_start -1 + nrow(events2[[q]])
#       
#       for (r in 1:2) {
#        
#         event_vector[idx_start:idx_stop,r] <- events2[[q]][,r+5]
#          
#       }
#       
#       idx_start <- idx_start + nrow(events2[[q]])
#     }
#     
#     event_vector <- rbind(event_vector, event_vector)
#     comp_vector <- matrix(0, nrow(event_vector), ncol(event_vector))
#     
#     for (z in 1:2) {
#       
#       idx_start <- ((z-1) * nrow(event_vector)/2) + 1
#       idx_stop <- z*(nrow(event_vector)/2)
#       
#       comp_vector[idx_start:idx_stop,1] <- event_vector[idx_start:idx_stop, z]
#       
#     }
#     
#     for (k in 1:length(events2)) {
#       
#       evMat <- event_vector == i
#       cpMat <- event_vector == i
#       
#       X <- matrix(0, 2, 2)
#       
#       for (l in 1:2) {
#        
#         X[1,1] <- sum(evMat[,1] & cpMat[,1])
#         X[1,2] <- sum(evMat[,1] & !cpMat[,1])
#         X[2,1] <- sum(!evMat[,1] & cpMat[,1])
#         X[2,2] <- sum(!evMat[,1] & !cpMat[,1])
#         
#         kappaMat[[i]][l,j] <- cohen.kappa(X)$kappa
#       }
#     }
#   }
# }
...
A2017.cod.acc <- lapply(A2017, function(x) {
  ...
  df <- df %>% dplyr::filter(label_MN < 4 & label_RA < 4)

  events <- lapply(1:3, function(e) {
    
    # df <- df %>% dplyr::filter(label_MN == e | label_RA == e)
...
A2017.conf <- cbind.data.frame(alg.names, lapply(1:length(A2017), function(stim) {
  ...
  conf.alg <- sapply(names(df)[-c(1,2)], function(alg) {
  ...
  conf.cod <- sapply(1:2, function(cod) {
  ...
  return(c(conf.cod, conf.alg))
}) %>% reduce(cbind))


# Create figure for disagreement ratio

names(A2017.conf) <- c("Algorithm", "Dots", "Image", "Video")

disag.df <- A2017.conf %>% pivot_longer(names(A2017.conf)[2:4], names_to = "Condition", values_to = "Ratio")
