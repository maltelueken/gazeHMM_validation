---
title             : "Classifying Gaze Data With an Unsupervised Generative Hidden Markov Model"
shorttitle        : "Classifying Gaze Data with an HMM"
...
abstract: |
  Eye-tracking allows researchers to infer cognitive processes from eye movements that are classified into distinct events. Parsing the events is typically done by algorithms. Previous algorithms have successfully used hidden Markov models (HMMs) for classification but can still be improved in several aspects. To address these aspects, we developed gazeHMM, an algorithm that uses an HMM as a generative model, has no critical parameters to be set by users, and does not require human coded data as input. The algorithm classifies gaze data into fixations, saccades, and optionally postsaccadic oscillations and smooth pursuits. We evaluated the performance of gazeHMM in a simulation study and on two benchmark data sets. The simulation study showed that gazeHMM successfully recovered HMM parameters and hidden state sequences. Critical exceptions for parameter recovery were including the smooth pursuit state in the generative model and even low noise levels. We assessed the performance on benchmark data by model comparing generative models with different events and agreement of the classification to human coding compared to other algorithms. The model comparison preferred more events in the generative models than we expected. Nevertheless, gazeHMM showed relatively high similarity to human coding and outperformed other algorithms in this aspect. For dynamic stimuli, our algorithm tended to switch rapidly between fixations and smooth pursuits. Thus, we recommend using gazeHMM for parsing smooth pursuits only in explorative settings. Future HMM algorithms could use covariates to better capture eye movement processes and explicitly model event durations to classify smooth pursuits more accurately.
