...
On one hand, many of these classification algorithms use non-parametric methods to differentiate between eye movement events. For instance, the classic "Velocity-threshold" algorithm classifies samples with a velocity above a fixed threshold as saccades [@Stampe1993]. On the other hand, many parametric methods have been developed recently. Some of them require human labeled training data as input and can therefore be termed as supervised. For example, @Bellet2019 trained a convolutional neural network (CNN) on eye-tracking data from humans and macaques and achieved classification results that were similarly accurate to those of human coders. However, these supervised methods have several disadvantages: First, they require a sufficient amount of labeled data (usually labeled by humans) as input, which can become costly and time consuming [@Zemblys2019]. Second, they often rely on hyperparameters which are difficult to theoretically justify. Third, most supervised methods operate in a black-box fashion, which implies that they do not make any explicit assumptions about the data generating process [cf. @Zembyls2019]. Beyond that, there are several algorithms using unsupervised techniques. They typically do not need to be trained on labeled data and are also more flexible in classifying data from different individuals, tasks, or eye-trackers [e.g., @Houpt2018; @Hessels2017].  
...
(ref:plot-algorithm-workflow) Example work flow for eye movement event classification algorithms: (a) the raw gaze signal in x (dark red) and y (dark blue) coordinates; (b) the raw gaze signal is filtered and transformed into a velocity signal; (c) samples are classified as events (indicated by colors), and (d) relabeled. Sequences of samples belonging to the same event are merged (indicated by black segments). Data from @Andersson2017.
...
p1 <- ggplot(toPlot, aes(x = t, x.va)) + 
  ...
  geom_point(aes(y = x.va), color = "darkred", size = 0.9) + 
  geom_point(aes(y = y.va), color = "darkblue", size = 0.9) +
...
p3 <- ggplot(toPlot, aes(x = t, y = vel)) + 
  geom_line() + 
  geom_point(aes(color = event.pre), size = 0.9) +
  scale_y_continuous(name = "Velocity (deg/s)", limits = c(0, 250), breaks = pretty(c(0, 250)))

p4 <- ggplot(toPlot, aes(x = t, y = vel)) + 
  geom_line() + 
  geom_point(aes(color = event.post), size = 0.9) +
  scale_y_continuous(name = "Velocity (deg/s)", limits = c(0, 250), breaks = pretty(c(0, 250)))
...
When we developed gazeHMM, the gamma distribution appeared to fit eye movement data well, but we also note that it might not necessarily be the best fitting distribution for every type of eye movement data. We assume that the best fitting distribution will depend on the task, eye-tracker, and individual (see discussion).  
...
## Results
### Parameter Variation
In the first part of the simulation, we examined how varying the parameters[^2] in the HMM affected the deviation of estimated parameters and accuracy of estimated state sequences. For the two-state HMM, the recovery of parameters and states was nearly perfect (all RMdSPDs < 0.1, intercepts and slopes of regression lines almost zero and one, respectively, and Cohen's kappa close to 1). We therefore chose to include the respective figures in the supplementary material.  
