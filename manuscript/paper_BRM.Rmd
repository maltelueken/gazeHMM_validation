...
Most of these classification algorithms use non-parametric methods to differentiate between eye movement events. For instance, the classic "Velocity-Threshold" algorithm classifies samples with a velocity above a fixed threshold as saccades [@Stample1993]. More recently, supervised statistical learning techniques have been successfully used to parse eye movement data. For example, @Bellet2019 trained a convolutional neural network on eye-tracking data from humans and macaques and achieved classification results that were similarly accurate to those of human coders. However, these supervised methods have several disadvantages: First, they require labeled data (usually labeled by humans) as input. Second, they often rely on hyperparameters which are difficult to theoretically justify. Third, their parameters are usually difficult to interpret: Most supervised methods operate in a black-box fashion, which implies that they don't make any explicit assumptions about the data generating process. Oppositely, several algorithms using unsupervised techniques have been developed. They typically don't need to be trained on labeled data and are thus more flexible in classifying data from different individuals, tasks, or eye-trackers.  
Besides differentiating between supervised and unsupervised methods, algorithms can vary in whether they are explicitly modeling the data generating process and are thus able to simulate new data. To our knowledge, these *generative models* have been rarely used to classify eye movement data [cf. @Mihali2017, @Wadehn2019]. However, they come with various advantages and we will summarize the two most important for eye movement classification here [for an overview, see @Epstein2008].  
First, generative models allow to not only predict (classify) but also explain eye-tracking phenomena. For instance, they can inhere parameters that explain why oscillations only occur after but not before saccades. When the goal is to understand eye movements and improve their classification based on this understanding, this aspect is clearly an advantage over non-parametric or supervised methods. Second, generative models can challenge prevailing theoretical assumptions and bring up new research questions. For example, they might suggest that oscillations also occur before saccadic eye movements [as has been mentioned by @Nystrom2010] or that the assumption of discrete eye movements does not hold [as has been discussed in @Andersson2017; maybe others].  
