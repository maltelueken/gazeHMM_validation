...
(@) All samples within +/- *blink window* around blink indicated samples are marked as invalid and labeled as blinks.
...
(@) Velocities and accelerations for the x- and y-coordiantes are calculated using an SG filter (3rd order, length = 5).
(@) Combined velocitiy and acceleration are calculated as: $v = f \sqrt{v_x^2 + v_y^2}$ and $a = f \sqrt{a_x^2 + a_y^2}$, with $f$ as the sampling rate. Note that acceleration is only multiplied by the sampling rate once. The second multiplication is done during postprocessing.
(@) Velocity and acceleration of value zero are nudged to be 0.01.
(@) Samples with velocity above 1000 deg/s or acceleration above 100 deg/s are marked as invalid.
